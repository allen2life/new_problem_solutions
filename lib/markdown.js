...
function mapFenceLanguage(info) {
  ...
  if (lang === 'c++' || lang === 'c' || lang === 'cpp') return 'clike';
  return lang;
}
...
function rbookFenceRendererPlugin(md) {
  ...
  md.renderer.rules.fence = (tokens, idx, options, env, self) => {
    ...
    const lang = mapFenceLanguage(rawInfo);
    ...
    const vizReg = /^viz($|-\w+)/;
    if (rawInfo === 'dot' || rawInfo === 'graphviz' || vizReg.test(rawInfo)) {
      return `<div class="graphviz"><pre class="dot">\n${escaped}</pre></div>\n`;
    }

    if (rawInfo && Prism.languages[lang]) {
      const highlighted = Prism.highlight(token.content, Prism.languages[lang], lang);
      return `<div class="language-${md.utils.escapeHtml(lang)} line-numbers-mode">`
        + `<div class="code-info-header"><span>${md.utils.escapeHtml(rawInfo)}</span></div>`
        + `<pre><code class="language-${md.utils.escapeHtml(rawInfo)}">${highlighted}</code></pre>`
        + '</div>\n';
    }
