...
start char : ${state.src[start]}
    ...
    /* another attrs */
    {
      let openParen = params.indexOf('(')
      let closeParen = params.lastIndexOf(')')
      let raw = openParen >= 0 && closeParen > openParen
        ? params.substring(openParen + 1, closeParen)
        : ''
      let params_list = raw.split(',').filter(Boolean)
