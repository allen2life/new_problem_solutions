...
def main():
    ...
    if '-h' in sys.argv[1:] or '--help' in sys.argv[1:]:
        parser = argparse.ArgumentParser(
            ...
            epilog="""使用示例:
  dot2png.py                    # 转换当前目录下的所有dot文件
  dot2png.py /path/to/dots      # 转换指定目录下的所有dot文件
  dot2png.py -O /path/to/output # 指定输出目录
  cat file.dot | dot2png.py -o output.png  # 管道模式""",
            formatter_class=argparse.RawDescriptionHelpFormatter
        )
        parser.add_argument("input_path", nargs="?", default=".", help="包含dot文件的输入目录")
        parser.add_argument("-o", "--output", default="output.png", help="管道模式下的输出PNG文件路径")
        parser.add_argument("-O", "--output_path", default=".", help="文件模式下的输出目录")
        parser.parse_args()
        return
